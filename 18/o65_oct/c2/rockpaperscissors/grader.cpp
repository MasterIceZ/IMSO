...
void invalid_input(std::string s="INVALID INPUT!!!!"){
	printf("%s\n", s.c_str());
}
...
int tournament(std::vector<int>person_index){
  ...
  for(int i=0;i<person_index.size();i++){
    if(person_index[i] < 0 || person_index[i] > 3*n){
      invalid_input("index out of bound");
      return -1;
    }
  ...
  if(same_index_check.size() != person_index.size()){
    invalid_input("there are same person in vector!");
    return -1;
  }
  if(same_index_check.size()<3){
    invalid_input("size of vector is less than 3");
    return -1;
  }
  tournament_call++;
  ...
  while(q--){
    ...
    if(t==0){
      for(int i=0,x;i<(n*3)+1;i++){
        ...
        if(x==0){
          if(isonecheater || i == 0){
            invalid_input("there are more than 1 cheater or cheater is at 0");
            return 0;
          }
          isonecheater = true;
          cheater_position = i;
          ...
          invalid_input("x is out of bound");
          return 0;
        }
      }
