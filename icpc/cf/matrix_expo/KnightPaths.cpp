...
const int di[] = {-2, -1, -2, -1, 2, 1, 2, 1};
const int dj[] = {-1, -2, 1, 2, -1, -2, 1, 2};
...
signed main(int argc, char *argv[]) {
	...
	cin >> k;
	k++;
	matrix_t<mint, 64> base;
	matrix_t<mint, 64> answer = matrix_t<mint, 64>::identity();
	auto make_index = [&](int i, int j) {
		return i * 8 + j;
	};
	for(int i=0; i<8; ++i) {
		for(int j=0; j<8; ++j) {
			for(int k=0; k<8; ++k) {
				...
				base[make_index(i, j)][make_index(ii, jj)] = 1;
			}
		}
	}
	for(int i=0; i<=64; ++i) {
